...
public class Polygons {
     ...
     * @param poly
     *
     * @return true if point with x, y in poly of polx nad poly coordinates else false
     */
    public static final boolean isPointInPolygon( double testX, double testY, double[] polx, double[] poly ) {
        //this method uses the ray tracing algorithm to determine if the point is in the polygon
        int nPoints = polx.length;
        int j = -999;
        int i = -999;
        boolean locatedInPolygon = false;
        for ( i = 0; i < ( nPoints ); i++ ) {
            ...
            double vertY_i = poly[ i ];
            double vertX_i = polx[ i ];
            double vertY_j = poly[ j ];
            double vertX_j = polx[ j ];
    ...
    public static final boolean PointIsInPoly( double[] pnt, double[] polyx, double[] polyy ) {
        return PointIsInPoly(pnt[0], pnt[1], polyx, polyy );
    }

    public static final boolean PointIsInPoly( double pntx, double pnty, double[] polyx, double[] polyy ) {
        int i, j;
        boolean res = false;
