...
public class ExifThumbnailReader
{
   ...
   * thumbnail cannot be found this returns <code>null</code> without throwing an exception.
   * <p>Not all JPEG files will have thumbnails, but it is becoming more common
  ...
  /**
   * This extracts the thumbnail from a large JPEG file, if it can be found.  If
   * a thumbnail cannot be found this returns null without throwing an exception.
   * <p>Not all JPEG files will have thumbnails, but it is becoming more common
   * for large photos to have embedded thumbnails.
   *
   * @return the jpeg file of this thumbnail if it can be read, or null.
   ...
   * thumbnail cannot be found this returns <code>null</code> without throwing an exception.
   * <p>Not all JPEG files will have thumbnails, but it is becoming more common
   * for large photos to have embedded thumbnails. <p>The
   * <code>InputStream</code> is not closed by this method.
   *
   * @return the jpeg file of this thumbnail if it can be read, or null.
   */
  ...
  /**
   * @param input        a large JPEG file, possibly with an embedded thumbnail.
   * @param scratchArray an array of at least 128 bytes to use temporarily.
   *
   * @return
   *
    ...
    throws IOException
  {
    ...
    if ( !( b[ 0 ] == -1 && b[ 1 ] == -40 ) )
    {
      ...
      if ( debug ) System.err.println( "error 1" );
      return null;
    }
    ...
    if ( !( b[ 0 ] == -1 && b[ 1 ] == -31 ) )
    {
      ...
      if ( debug ) System.err.println( "error 1.5" );
      return null;
    }
    ...
    {
      //this didn't have "Exif" followed by a 0x00, so this isn't an exif file.
      if ( debug ) System.err.println( "error 2" );
      return null;
    }
    ...
    else if ( b[ 0 ] == 77 && b[ 1 ] == 77 )
    { //big endian
      //do nothing
    }
    else
    {
      if ( debug ) System.err.println( "error 3: " + b[ 0 ] + " " + b[ 1 ] );
      return null;
    }

    read( in, b, 2, reverse );
    if ( !( b[ 0 ] == 0 && b[ 1 ] == 42 ) )
    { //required byte in TIFF header
      if ( debug ) System.err.println( "error 4" );
      return null;
    }
    read( in, b, 4, reverse ); //position of zero-eth IFD

    //the position of the zero-eth IFD
    long pos = ( b[ 0 ] & 0xff ) * 256 * 256 * 256 +
      ( b[ 1 ] & 0xff ) * 256 * 256 +
      ( b[ 2 ] & 0xff ) * 256 +
      ( b[ 3 ] & 0xff );
    ...
    pos = ( b[ 0 ] & 0xff ) * 256 * 256 * 256 +
      ( b[ 1 ] & 0xff ) * 256 * 256 +
      ( b[ 2 ] & 0xff ) * 256 +
      ( b[ 3 ] & 0xff );

    i0.resolveIFDs( in, reverse, offset );
    if ( debug ) System.out.println( i0 );
    ...
    if ( i1 != null && i1.getJPEGPointer() > 0 )
    {
      ...
      if ( len == 0 )
      {
        if ( debug ) System.err.println( "error 4.5" );
        return null;
      }
      ...
      if ( !( thumbnailData[ 0 ] == -1 && thumbnailData[ 1 ] == -40 ) )
      {
        ...
        if ( debug ) System.err.println( "error 5" );
        return null;
      }
      byte[] d = new byte[4];
  ...
  /**
   * This extracts the thumbnail from a large JPEG, if it can be found.  If a
   * thumbnail cannot be found this returns null without throwing an exception.
   * <p>Not all JPEG files will have thumbnails, but it is becoming more common
   * for large photos to have embedded thumbnails.
   *
   * @return the jpeg of this thumbnail if it can be read, or null.
  ...
  public static BufferedImage getThumbnail( InputStream in )
  {
    try
    {
      ...
      InputStream jpegIn = new CombinedInputStream(
        ...
        new ByteArrayInputStream( thumbnailData, 2, thumbnailData.length - 2 )
      );
   ...
   *
   * @param array an array of at least 42 bytes to write to.
   *
  ...
  private static File writeJPEGFile( byte[] array1, int array1offset,
                                     int array1size, byte[] array2,
                                     int array2offset, int array2size )
    throws IOException
  {
