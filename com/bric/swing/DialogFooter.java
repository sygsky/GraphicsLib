 ...
 * action that the user is most likely to perform if that action isnÕt
 ...
 * the user performs when pressing the buttonÑfor example, Erase, Save, or Delete."
...
public class DialogFooter extends JPanel {	
    ...
    public static DialogFooter createDialogFooter(JComponent[] leftComponents,int options,int defaultButton,int escapeKeyBehavior) {
    	...
    	if(options==CANCEL_OPTION) {
    		...
    		dontSaveButton.putClientProperty(DialogFooter.PROPERTY_UNSAFE, Boolean.TRUE);
    		if(JVM.isMac) {
	    		dismissControls = new JButton[] { saveButton, cancelButton, dontSaveButton };
    		} else {
