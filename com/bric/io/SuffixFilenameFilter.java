...
public class SuffixFilenameFilter extends javax.swing.filechooser.FileFilter implements FilenameFilter, java.io.FileFilter {
	...
	private static void main(String[] args) {
		...
		System.out.println(new SuffixFilenameFilter(new String[] {"gif", "jpeg"}));
		System.out.println(new SuffixFilenameFilter(new String[] {"gif", "jpeg", "png"}));
		System.out.println(new SuffixFilenameFilter(new String[] {"gif", "jpeg", "png", "bmp"}));
		System.out.println(new SuffixFilenameFilter(new String[] {"gif", "jpeg", "png", "bmp", "tiff"}));
	}
	
	/** The localized strings used in the description. */
	public static ResourceBundle strings = ResourceBundle.getBundle("com.bric.io.SuffixFilenameFilter");
	...
	public String getDescription() {
		...
		try {
			if(suffix.length==1) {
				...
				text = Text.replace(text, "^0", suffix[0].toUpperCase());
				return text;
			} else if(suffix.length==2) {
				String text = strings.getString("twoFiles");
				text = Text.replace(text, "^0", suffix[0].toUpperCase());
				text = Text.replace(text, "^1", suffix[1].toUpperCase());
				return text;
			} else {
